 ...
 * changed_modified_2_1 changed_modified_2_2">FacesServlet</strong> is a servlet
 * that manages the request processing lifecycle for web applications that are
 * utilizing JavaServer Faces to construct the user interface.
 * </p>
 *
 ...
 * runtime must provide an implementation of the
 * {@link javax.servlet.ServletContainerInitializer} interface that declares the
 * following classes in its {@link javax.servlet.annotation.HandlesTypes}
 * annotation.
 * </p>
 * 
 * <ul>
 ...
 * 
 * <p>
 * This servlet must automatically be mapped if it is <strong>not</strong>
 * explicitly mapped in <code>web.xml</code> or <code>web-fragment.xml</code>
 ...
 * <li>
 * <p>
 * A <code>faces-config.xml</code> file is found in the <code>META-INF</code>
 ...
 * <li>
 * <p>
 * A filename ending in <code>.faces-config.xml</code> is found in the
 * <code>META-INF</code> directory of a jar in the application's classpath.
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * The <code>javax.faces.CONFIG_FILES</code> context param is declared in
 * <code>web.xml</code> or <code>web-fragment.xml</code>.
 * </p>
 * </li>
 * 
 * <li>
 * <p>
 * The <code>Set</code> of classes passed to the <code>onStartup()</code> method
 * of the <code>ServletContainerInitializer</code> implementation is not empty.
 * </p>
 * </li>
 * 
 * </ul>
 ...
 * must be mapped to the following &lt;<code>url-pattern</code>&gt; entries.
 * </p>
 * 
 * <ul>
 * 
 ...
 * {@code javax.servlet.annotation.MultipartConfig}. This causes the Servlet
 * container in which the JSF implementation is running to correctly handle
 * multipart form data.
 * </p>
 * 
 * <p>
 ...
 * 
 * <p>
 * The topic of web application security is a cross-cutting concern and every
 * aspect of the specification address it. However, as with any framework, the
 * application developer needs to pay careful attention to security. Please
 * consider these topics among the rest of the security concerns for the
 * application. This is by no means a complete list of security concerns, and is
 * no substitute for a thorough application level security review.
 * </p>
 *
 * <ul>
 * 
 ...
 * 
 * <p>
 * If the <code>FacesServlet</code> is mapped using a prefix
 * <code>&lt;url-pattern&gt;</code>, such as
 * <code>&lt;url-pattern&gt;/faces/*&lt;/url-pattern&gt;</code>, something must
 * be done to prevent access to the view source without its first being
 * processed by the <code>FacesServlet</code>. One common approach is to apply a
 * &lt;security-constraint&gt; to all facelet files and flow definition files.
 * Please see the <strong>Deployment Descriptor</strong> chapter of the Java
 * Servlet Specification for more information the use of
 * &lt;security-constraint&gt;.
 * </p>
 * 
 * <p>
 ...
 * &lt;http-method&gt; and &lt;http-method-omission&gt; elements. Please see the
 * <strong>Security</strong> of the Java Servlet Specification for more
 * information the use of these elements.
 * </p>
 * 
 * 
 * </ul>
...
public final class ComsatFacesServlet extends FiberHttpServlet {
	...
	/*
	 * A white space separated list of case sensitive HTTP method names that are
	 * allowed to be processed by this servlet. * means allow all
	 */
	private static final String ALLOWED_HTTP_METHODS_ATTR = "com.sun.faces.allowedHttpMethods";

	// Http method names must be upper case.
	// http://www.w3.org/Protocols/HTTP/NoteMethodCS.html
	// List of valid methods in Http 1.1
	// http://www.w3.org/Protocols/rfc2616/rfc2616-sec9.html#sec9

	private enum HttpMethod {

		OPTIONS("OPTIONS"), GET("GET"), HEAD("HEAD"), POST("POST"), PUT("PUT"), DELETE("DELETE"), TRACE(
	 ...
	 * <code>/WEB-INF/faces-config.xml</code> which is loaded automatically if
	...
	/**
	 * From GLASSFISH-15632. If true, the FacesContext instance left over from
	 * startup time has been released.
	 */
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		System.out.println("Back to servlet init"); 
		...
		try {
			...
			System.out.println("lifecycle=" + lifecycleId);
	...
	private void initHttpMethodValidityVerification() {
		...
		String[] methods;
		String allowedHttpMethodsString = getServletConfig().getServletContext().getInitParameter(ALLOWED_HTTP_METHODS_ATTR);
		if (null != allowedHttpMethodsString) {
			methods = allowedHttpMethodsString.split("\\s+");
			assert(null != methods); // assuming split always returns a non-null
										// array result
...
//					}
			...
			if (5 == allowedKnownHttpMethodsStringList.size()) {
				...
				List<HttpMethod> restList = new ArrayList<HttpMethod>(allowedKnownHttpMethodsStringList.size() - 1);
	 ...
	 * <span class="changed_modified_2_2">Process</span> an incoming request,
	 ...
	 *
	 * <p>
	 * This method must respond to requests that
	 * <span class="changed_modified_2_2">contain</span> the following strings
	 * by invoking the <code>sendError</code> method on the response argument
	 ...
	 * 
	 * <p>
	 * If none of the cases described above in the specification for this method
	 * apply to the servicing of this request, the following action must be
	 ...
	 * {@link javax.faces.application.ResourceHandler#isResourceRequest}. If
	 * this returns <code>true</code> call
	 * {@link javax.faces.application.ResourceHandler#handleResourceRequest}. If
	 * this returns <code>false</code>, <span class="changed_added_2_2">call
	 ...
	 * {@link javax.faces.lifecycle.Lifecycle#render}. If a
	 * {@link javax.faces.FacesException} is thrown in either case, extract the
	 ...
	 * <code>FacesException</code> instance as the root cause, then rethrow the
	 * <code>ServletException</code> instance. If the cause is an instance of
	 * <code>ServletException</code>, rethrow the cause. If the cause is an
	 * instance of <code>IOException</code>, rethrow the cause. Otherwise,
	 * create a new <code>ServletException</code> instance, passing the message
	 * from the cause, as the first argument, and the cause itself as the second
	 * argument.
	 * </p>
	 * 
	 * <p class="changed_modified_2_0_rev_a">
	 * The implementation must make it so
	 * {@link javax.faces.context.FacesContext#release} is called within a
	 * finally block as late as possible in the processing for the JSF related
	...
	@Suspendable
	public void serviceFacesContext(ServletRequest req, ServletResponse resp) throws IOException, ServletException {
		HttpServletRequest request = (HttpServletRequest) req;
		HttpServletResponse response = (HttpServletResponse) resp;

		System.out.println("serviceFacesContext"); 
		System.out.println(getServletConfig());
...
//			}
			System.out.println("Thread "+Thread.currentThread().getName()+" given to FacesServlet.service() in interrupted state");
		}
		...
		if (pathInfo != null) {
			...
			if (pathInfo.contains("/WEB-INF/") || pathInfo.contains("/WEB-INF") || pathInfo.contains("/META-INF/")
		...
		FacesContext context = facesContextFactory.getFacesContext(getServletConfig().getServletContext(), request, response,
				lifecycle);
		
		System.out.println("serviceFacesContext : got the FacesContext"); 
		...
		try {
			...
			if (handler.isResourceRequest(context)) {
				System.out.println("serviceFacesContext : handler.handleResourceRequest"); 
				System.out.println(handler);
				handler.handleResourceRequest(context);
			} else {

				System.out.println("serviceFacesContext : lifecycle.execute"); 
				System.out.println(context);
